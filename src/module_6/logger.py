...
class ModelLogger:
    def __init__(self, log_file=None):
        ...
        if not self.log_file.exists():
            with open(self.log_file, 'w') as f:
                ...
                f.write("=" * 50 + "\n")
                f.write("Format: TIMESTAMP | PREDICTION | user_id | prediction | latency | total_orders | input_features | feature_stats\n")
                f.write("=" * 50 + "\n")
    ...
    def log_complete_prediction(
        ...
        features_str = " | ".join([
            f"{name}={value:.3f}" for name, value in zip(feature_names, input_features.flatten())
        ])
    ...
    def log_error(self, user_id: str, error_type: str, error_message: str):
        ...
        timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        log_entry = f"{timestamp} | MODEL_ERROR | user_id={user_id} | type={error_type} | error={error_message}\n"
