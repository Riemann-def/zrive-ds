...
def process_data(
    data_frames: List[DataFrameType], 
    ...
    for city in cities:
        ...
        if "temperature_2m_mean" in city_df.columns:
            resampled["temperature_2m_mean"] = city_df["temperature_2m_mean"].resample(resample_freq).mean()
            # Add min/max temperature for temperature range analysis
            resampled["temperature_2m_min"] = city_df["temperature_2m_mean"].resample(resample_freq).min()
            resampled["temperature_2m_max"] = city_df["temperature_2m_mean"].resample(resample_freq).max()
            resampled["temperature_2m_range"] = resampled["temperature_2m_max"] - resampled["temperature_2m_min"]
            
        if "precipitation_sum" in city_df.columns:
            resampled["precipitation_sum"] = city_df["precipitation_sum"].resample(resample_freq).sum()
            # Count rainy days (days with precipitation > 1mm)
            resampled["rainy_days"] = (city_df["precipitation_sum"] > 1).resample(resample_freq).sum()
            # Calculate maximum single-day precipitation
            resampled["max_daily_precipitation"] = city_df["precipitation_sum"].resample(resample_freq).max()
            
        if "wind_speed_10m_max" in city_df.columns:
            resampled["wind_speed_10m_max"] = city_df["wind_speed_10m_max"].resample(resample_freq).max()
            resampled["wind_speed_10m_mean"] = city_df["wind_speed_10m_max"].resample(resample_freq).mean()
            # Count windy days (days with max wind speed > 20km/h)
            resampled["windy_days"] = (city_df["wind_speed_10m_max"] > 20).resample(resample_freq).sum()
        
        # Calculate moving averages for trend analysis
        if len(resampled) > 3:  # Need at least 3 points for moving average
            for var in VARIABLES:
                if var in resampled.columns:
                    resampled[f"{var}_ma3"] = resampled[var].rolling(window=3, min_periods=1).mean()
                    if len(resampled) > 12:  # For annual trends
                        resampled[f"{var}_ma12"] = resampled[var].rolling(window=12, min_periods=1).mean()
    ...
    for var in VARIABLES:
        if var in result_df.columns:
            ...
            result_df[f"{var}_rank"] = result_df.groupby("time")[var].rank(ascending=var!="precipitation_sum")
